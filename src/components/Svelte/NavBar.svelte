<script lang="ts">
  ...
  function handleNavClick(href: string) {
    // Extract the pathname from the href immediately
  ...
  onMount(() => {
    ...
    // Also check periodically in case navigation happens outside of clicks
    // This is a fallback for Astro's default navigation
...
>
  ...
  >
    <a
      ...
      class="font-cursive rounded-full border px-2 py-1 transition-all duration-100 text-2xl text-black/80 border-black/80 hover:shadow-lg hover:-rotate-10 ease-in-out hover:scale-105"
    >
      CN
    </a>
